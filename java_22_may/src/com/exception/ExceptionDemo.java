 ...
 * 
 * 2. runtime time error : Arithmetic exception, ArrayIndexOutpfNound, NumberFormatException etc..
 * 3. error : developer mistake 
 * 
 * keywords :
 * 1.try : 
 * 		1.if you doubt that a code may have error it should be written in try block
 * 		2.the code which have will be skipped and remaining code must be executed 
 * 		3.try throw the error to catch block
 * 		4.try must be followed by catch block ot finally or both
 * 2.catch :
